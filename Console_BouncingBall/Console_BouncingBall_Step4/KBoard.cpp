...
int KBoard::IsCollide(const KBlock& block_) const
{
    if (block_.m_worldPos.x <= 1 || block_.m_worldPos.x >= BOARD_SIZE_COL-1)
        return 1;
    else if (block_.m_worldPos.y <= 1 || block_.m_worldPos.y >= BOARD_SIZE_ROW-1)
        return 2;
