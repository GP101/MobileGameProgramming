...
public class VisibleSkeletonEd : Editor {
	...
	void DrawSkeleton(Transform root, Transform prevRoot, HashSet<Transform> avatarBones, Transform leftHandRoot, Transform rightHandRoot) {
		...
		Handles.SphereHandleCap(-1, root.position, Quaternion.identity, BONE_RADIUS, EventType.Ignore);
	...
	void DrawAxes(Transform root) {
        ...
        Handles.ArrowHandleCap(-1, root.position, Quaternion.LookRotation(root.right), AXIS_LENGTH, EventType.Ignore);

		Handles.color = Color.green;
        Handles.ArrowHandleCap(-1, root.position, Quaternion.LookRotation(root.up), AXIS_LENGTH, EventType.Ignore);

		Handles.color = Color.blue;
        Handles.ArrowHandleCap(-1, root.position, Quaternion.LookRotation(root.forward), AXIS_LENGTH, EventType.Ignore);
