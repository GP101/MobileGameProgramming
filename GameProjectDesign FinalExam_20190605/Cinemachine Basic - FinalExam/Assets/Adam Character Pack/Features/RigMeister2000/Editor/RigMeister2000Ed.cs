...
public class RigMeister2000Ed : Editor {
	...
	void OnSceneGUI() {
		...
		if(target.debugShowActiveJointGizmo && m_jointSelected) {
			...
			Handles.RectangleHandleCap(-1, m_jointSelected.position, Camera.current ? Camera.current.transform.rotation : m_jointSelected.rotation, 0.05f, EventType.Ignore);
		}
		...
		foreach(var constraint in target.constraints) {
			...
			if(constraint.usePositionConstraint) try {
				...
				if(pc.weightedTargetJoints.Length > 1) {
					foreach(var wtj in pc.weightedTargetJoints) {
						...
						Handles.CubeHandleCap(-1, wtj.joint.position, Quaternion.identity, 0.025f, EventType.Ignore);
					}
				}

				Handles.color = Color.cyan;
				Handles.CubeHandleCap(-1, sourcePos, Quaternion.identity, 0.025f, EventType.Ignore);
			} catch(System.Exception) {}
			...
			if(constraint.useOrientationConstraint) try {
				...
				foreach(var wtj in oc.weightedTargetJoints) {
					...
					Handles.DrawLine(sourcePos, wtj.joint.position);
					Handles.color = Color.magenta;
					Handles.SphereHandleCap(-1, wtj.joint.position, wtj.joint.rotation, 0.025f, EventType.Ignore);
					Handles.ArrowHandleCap(-1, wtj.joint.position, wtj.joint.rotation, 0.1f, EventType.Ignore);
				}

				Handles.color = Color.cyan;
				Handles.SphereHandleCap(-1, sourcePos, weightedTargetOri, 0.025f, EventType.Ignore);
				Handles.ArrowHandleCap(-1, sourcePos, weightedTargetOri, 0.1f, EventType.Ignore);
			} catch(System.Exception) { }
			...
			if(constraint.useLookAtConstraint) try {
				...
				Handles.ConeHandleCap(-1, sourcePos + sourceToTarget.normalized * 0.080f, sourceTargetOri, 0.0125f, EventType.Ignore);
				Handles.ArrowHandleCap(-1, sourcePos, sourceTargetOri, 0.1f, EventType.Ignore);

				Handles.color = ms_axisColor[(int)lac.upSourceAxis];
				if(lac.upWorld) {
					Handles.ArrowHandleCap(-1, sourcePos, Quaternion.LookRotation(Vector3.up), 0.1f, EventType.Ignore);
				} else {
					var sourceToUp = lac.upJoint.position - sourcePos;
					Handles.ArrowHandleCap(-1, sourcePos, Quaternion.LookRotation(sourceToUp), 0.1f, EventType.Ignore);
				}
			} catch(System.Exception) {}
		}		
	}
